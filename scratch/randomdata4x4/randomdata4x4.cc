...
Vector randomvelocity()
{
  ...
  // const std::vector<double> velocities{sqrt(2)/2, 9*sqrt(2)/2, 22*sqrt(2)/2, 33*sqrt(2)/2, 100*sqrt(2)/2};
...
void simulation(Vector uepos, Vector gnbpos, Vector uevel, std::vector<int> seeds, double time, int trial, std::string path)
{
  ...
  // 1. specify the path of the file containing the codebook
  ptr_mmWave->SetEnbBeamformingCodebookAttribute ("CodebookFilename", StringValue (path + "/src/mmwave/model/Codebooks/4x4.txt"));
  ptr_mmWave->SetEnbPhasedArrayModelAttribute ("NumRows", UintegerValue (4));
  // 2. set the antenna dimensions
  ptr_mmWave->SetEnbPhasedArrayModelAttribute ("NumColumns", UintegerValue (4));
...
int main (int argc, char *argv[])
{
  ...
  // uepos=Vector(20.7916, 20.5237, zue);
