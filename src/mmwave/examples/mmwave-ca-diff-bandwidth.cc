...
static void 
CourseChange (Ptr<OutputStreamWrapper> stream, Ptr<const MobilityModel> position)
{
  Vector pos = position->GetPosition ();
  *stream->GetStream () << Simulator::Now () << ", pos=" << position << ", x=" << pos.x << ", y=" << pos.y
            << ", z=" << pos.z;
}
...
main (int argc, char *argv[])
{
  ...
  Config::ConnectWithoutContext (
    ...
    MakeBoundCallback(&CourseChange, stream));
