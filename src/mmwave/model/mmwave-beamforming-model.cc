...
MmWaveCodebookBeamforming::GetTypeId ()
{
  static TypeId
    ...
    .AddAttribute ("UpdatePeriod",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("Trial", "Trial counter", UintegerValue(0),
                  MakeUintegerAccessor (&MmWaveCodebookBeamforming::trial),
                  MakeUintegerChecker<uint32_t>());
  ;
  return tid;
}
...
MmWaveCodebookBeamforming::SetBeamformingVectorForDevice (Ptr<NetDevice> otherDevice, Ptr<PhasedArrayModel> otherAntenna)
{
  ...
  if (it != m_codebookIdsCache.end ())
  {
    ...
    NS_LOG_DEBUG (this << " found an entry in the map for antenna " << otherAntenna <<
                   "\tCodebook index for this antenna " << it->second.thisCbIdx <<
                   "\tCodebook index for other antenna " << it->second.otherCbIdx <<
                   "\tLast update " << it->second.lastUpdate.GetSeconds() << " s " <<
                   "\tNow " << Simulator::Now ().GetSeconds () << ", update? " << update);
  }
  ...
  if (notFound || update)
  {
    ...
    // std:: cout << powerMatrix.size () << std::endl;
    ...
    if(print_val==true)
    {
      ...
      std::cout << "\t" << thisCbIdx << "\t" << otherCbIdx << "\t" << *argMaxIt << "\t" << trial << std::endl;
    }
...
MmWaveCodebookBeamforming::ComputeBeamformingCodebookMatrix (Ptr<NetDevice> otherDevice, Ptr<PhasedArrayModel> otherAntenna) const
{
  ...
  Ptr<MobilityBuildingInfo> buildingInfoA = thisMob->GetObject<MobilityBuildingInfo> ();
  // Ptr<MobilityBuildingInfo> buildingInfoA = otherMob->GetObject<MobilityBuildingInfo> ();

  std::cout << std::setprecision(30) << std::fixed;

  // std::cout << thisMob->GetPosition().x<< "\t" << thisMob->GetPosition().y << "\t" << thisMob->GetPosition().z << "\t" << thisMob->GetVelocity().x << "\t" << thisMob->GetVelocity().y << "\t" << thisMob->GetVelocity().z;
  if(thisMob->GetPosition().x!=0 && thisMob->GetPosition().y!=0)
  {
    std::cout << thisMob->GetPosition().x<< "\t" << thisMob->GetPosition().y << "\t" << thisMob->GetPosition().z << "\t" << thisMob->GetVelocity().x << "\t" << thisMob->GetVelocity().y << "\t" << thisMob->GetVelocity().z;
    print_val=true;
    // if (buildingInfoA->IsIndoor()==false)
    // {
    //   std::cout << thisMob->GetPosition().x<< "\t" << thisMob->GetPosition().y << "\t" << thisMob->GetPosition().z << "\t" << thisMob->GetVelocity().x << "\t" << thisMob->GetVelocity().y << "\t" << thisMob->GetVelocity().z;
    //   print_val=true;
    // }
  ...
  // std::cout << thisMob->GetPosition().x<< "\t" << thisMob->GetPosition().y << "\t" << thisMob->GetPosition().z << "\t" << thisMob->GetVelocity().x << "\t" << thisMob->GetVelocity().y << "\t" << thisMob->GetVelocity().z << "\t" << buildingInfoA->IsIndoor();
  // std::cout << thisCodebook->GetCodebookSize () << "\t" << otherCodebook->GetCodebookSize () << std::endl;
  ...
  // std:: cout << "Matrix of size " << matrix.size () << "x" << matrix[0].size () << std::endl;
